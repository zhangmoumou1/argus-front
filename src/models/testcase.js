...
const sortDirectoryTree = (nodes = []) => {
  return [...nodes]
    .sort((a, b) => {
      const sa = typeof a.sort_index === 'number' ? a.sort_index : 0;
      const sb = typeof b.sort_index === 'number' ? b.sort_index : 0;
      if (sa !== sb) {
        return sa - sb;
      }
    ...
    .map((item) => ({
      ...item,
      children: sortDirectoryTree(item.children || []),
    }));
};
  ...
  effects: {
    * listTestcaseDirectory({payload}, {call, put}) {
      ...
      if (auth.response(res)) {
        const sortedTree = sortDirectoryTree(res.data || []);
        yield put({
          type: 'save',
          payload: {
            directory: sortedTree,
            currentDirectory: sortedTree.length > 0 ? [sortedTree[0].key] : []
          }
        })
      }
    },
    ...
    * queryTestcaseDirectory({payload}, {call, put, select}) {
      const res = yield call(queryTestcaseDirectory, payload);
      if (auth.response(res)) {
        const expectedDirectoryId = parseInt(payload.directory_id, 10);
        const {currentDirectoryId} = yield select(state => state.testcase);
        if (!Number.isNaN(expectedDirectoryId) && currentDirectoryId !== null && expectedDirectoryId !== currentDirectoryId) {
          return;
        }
        yield put({
          type: 'save',
    ...
    * queryTestcase({payload}, {call, put, select}) {
      const res = yield call(queryTestCase, payload);
      if (auth.response(res)) {
        const expectedCaseId = parseInt(payload.caseId || payload.case_id, 10);
        const actualCaseId = parseInt(res?.data?.case?.id, 10);
        const {currentCaseId} = yield select(state => state.testcase);
        if (!Number.isNaN(expectedCaseId) && currentCaseId !== null && expectedCaseId !== currentCaseId) {
          return;
        }
        if (!Number.isNaN(expectedCaseId) && !Number.isNaN(actualCaseId) && expectedCaseId !== actualCaseId) {
          return;
        }
        yield put({
          type: 'save',
