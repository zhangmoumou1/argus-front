...
const TestCaseBottom = ({
  ...
  const lastVarsQueryKeyRef = useRef('');
  ...
  useEffect(() => {
    ...
    const steps = (preConstructor || []).filter((item) => item && item.case_id && item.name);
    const queryKey = JSON.stringify(steps.map((item) => ({case_id: item.case_id, name: item.name})));
    if (lastVarsQueryKeyRef.current === queryKey) {
      return;
    }
    lastVarsQueryKeyRef.current = queryKey;
    onQueryCaseVars(steps);
  }, [case_id, preConstructor]);
          ...
          tabBarExtraContent={
            <Dropdown menu={{items: envItems}}>
              <a onClick={(e) => {
                ...
                <Space>
                  {(envList.find((item) => item.id === executeEnv || item.id.toString() === executeEnv?.toString()) || {}).name || "选择当前执行环境"}
                  <DownOutlined/>
                </Space>
              </a>
            </Dropdown>
