...
const ProjectRole = ({project, roles, users, fetchData}) => {
  ...
  const permission = (item) => {
    if (item.project_role === 'OWNER') {
      return [<Tag key="owner-tag" color='blue' size="large">负责人</Tag>];
    }
    return [
      <Select key="role-select" style={{width: 80}} value={CONFIG.PROJECT_ROLE_MAP[item.project_role]} onChange={(data) => {
          ...
          renderItem={item => (
            <List.Item actions={permission(item)}>
              <Skeleton avatar title={false} loading={item.loading} active>
                <List.Item.Meta
                  avatar={<Avatar
                    src={getAvatarByUser(userMap[item.user_id])}/>}
